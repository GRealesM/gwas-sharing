################################################################################
### Preparing data for GWAS summary statistics sharing project. Revision 2  ####
################################################################################
...
## Extract dataset information for Table S1
s1 <- pubs
# fwrite(s1, "../tables/S1_studies_with_accessions.tsv", sep = "\t")
...
# GWAS catalog to double-check if they actually shared data.
...
f <- fread("../data/data_found.txt", header = F)$V1 %>% gsub("data/([0-9]+).xml", "\\1", ., perl = TRUE) %>% unique %>% as.numeric
...
### This is an updated dataset, with SJR data by year (see a)
md <- fread("../data/R1_Main_data_20221020.tsv")
...
# Save it
fwrite(md ,"../data/R1_Main_data_20221208.tsv", sep = "\t")


# Update non-sharer table to generate table S3
ns <- fread("../data/Nonsharers_all.tsv")
ns <- merge(ns, f, by="PMID", all.x = TRUE)
ns[ is.na(Sharer_updated), Sharer_updated:="N"]
table(ns$Sharer_updated) # 217 new sharers
ns <- ns[order(year)]

fwrite(ns , "../tables/S3_mislabelled_updated.tsv", sep = "\t")
