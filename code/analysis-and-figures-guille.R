...
bot=ggplot(x[year<=2022], aes(x=year,y=y,col=Public_ss)) +
 ...
 # geom_jitter(width=0.2,height=0) +
  geom_smooth() +
  scale_colour_seaborn("Sumstats", guide = guide_legend(reverse=TRUE), labels=c("No", "Yes")) +
  background_grid(major="y") +
  theme(legend.position=c(0.1,1),legend.justification=c(0,1),legend.direction = "horizontal") +
  labs(x="Publication year",y="log relative citation ratio")
...
ggsave("../figures/fig-mean-citation-count-per-year.tiff",height=8,width=11,units="in", bg ="white")
...
m10=glm(y~year + log(SJR) + sjournal + human + animal + molecular_cellular, data=x[!is.na(SJR) & year<2021])# %>% summary()
m11=glm(y~year + log(SJR) + sjournal + molecular_cellular, data=x[!is.na(SJR) & year<2021])# %>% summary()
m12=glm(y~year + log(SJR) + sjournal + human + molecular_cellular, data=x[!is.na(SJR) & year<2021])# %>% summary()
BIC(m5,m6,m7,m8,m9, m10, m11, m12) # adding human also helpful

## check all vars still useful - yes
drop1(m11)
m11a=glm(y~year + log(SJR) + molecular_cellular , data=x[!is.na(SJR) & year<2021])# %>% summary()
BIC(m11,m11a)

## go with m11a, lowest BIC
m11a=glm(y~year + log(SJR) + sjournal + molecular_cellular + Public_ss, data=x[!is.na(SJR) & year<2021])# %>% summary()
BIC(m11,m11a)

summary(m11a)
## conclude sharing has highly significant effect (t=12.146, p<2e-16, coef=0.559961)
summary(m11a)$coeff["Public_ssY",,drop=FALSE]
cat("Ratio of RCR in publications that shared vs did not = ",
exp(coef(m11a)["Public_ssY"]),
"\n95% CI: ",
exp(coef(m11a)["Public_ssY"]+ c(-1,1) * 1.96 * sqrt(vcov(m6a)["Public_ssY","Public_ssY"])),
"\n")
